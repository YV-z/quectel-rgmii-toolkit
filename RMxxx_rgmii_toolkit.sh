...
if echo "$ARCH" | grep -q "aarch64"; then
    cd /tmp && wget -O RM55x_rcPCIe_toolkit.sh https://ghgo.xyz/https://raw.githubusercontent.com/iamromulan/quectel-rgmii-toolkit/SDXPINN/RM55x_rcPCIe_toolkit.sh && chmod +x RM55x_rcPCIe_toolkit.sh && ./RM55x_rcPCIe_toolkit.sh && cd /
    exit 0
...
echo "                                           :+##+.            "
    ...
    echo "Visit https://ghgo.xyz/https://github.com/iamromulan for more!"
    echo -e "\e[0m"
    ...
    case $choice in
		...
		12) 
			...
			mkdir /usrdata/root
     	    mkdir /usrdata/root/bin
			cd /usrdata/root/bin
     	    wget -O fast https://ghgo.xyz/https://github.com/ddo/fast/releases/download/v0.0.4/fast_linux_arm && chmod +x fast
     	    cd /
